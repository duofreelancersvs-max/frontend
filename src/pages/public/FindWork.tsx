...
const FindWork = () => {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      const params = {
        ...
        locationType: selectedLocation !== "All Locations" ? selectedLocation.toLowerCase() : undefined,
      };
  ...
  const handleApply = (projectId: string) => {
    ...
    navigate(`/freelancer/projects`, { state: { applyToProjectId: projectId } });
  };
      ...
      <section className="relative pt-28 pb-16 md:pt-32 md:pb-24 overflow-hidden bg-white dark:bg-[#050B15] border-b border-slate-200 dark:border-white/5">
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 pb-10 border-b border-slate-100 dark:border-white/5">
            <div>
              ...
              <h1 className="text-5xl md:text-7xl font-black text-navy dark:text-white mb-6 leading-tight">
                Find Your <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal to-royal-blue dark:from-teal-light dark:to-sky-blue">Next Project</span>
              </h1>
              <p className="text-xl text-slate-500 dark:text-slate-400 max-w-2xl leading-relaxed font-medium">
                Browse thousands of high-quality freelance opportunities across all creative domains.
              </p>
            </div>
            <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 px-6 py-4 rounded-[2rem] shadow-xl shadow-slate-200/50 dark:shadow-none self-start md:self-auto flex items-center gap-4">
              ...
              <div>
                <div className="text-2xl font-black text-navy dark:text-white tracking-tighter">{totalCount}</div>
                <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Live Projects</div>
              </div>
            </div>
          </div>
          ...
          <div className="relative group max-w-4xl">
            <Search size={22} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-teal transition-colors" />
            <input
              type="text"
      ...
      <main className="py-12 container mx-auto px-4 lg:px-8">
        <div className="flex flex-col gap-4 mb-6">
          <div className="flex items-center gap-2 overflow-x-auto pb-2 px-1 no-scrollbar">
            ...
            >
              ...
              Filters
              {(selectedCategory !== "All Categories" || selectedLevel !== "All Levels" || selectedLocation !== "All Locations") && (
          ...
          {showFilters && (
            <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 animate-in slide-in-from-top-2 duration-200">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Experience Level</label>
                  <select 
                    value={selectedLevel}
              ...
              <div className="mt-6 flex justify-end gap-3 border-t border-slate-100 dark:border-white/5 pt-6">
                ...
                <Button size="sm" className="bg-teal text-white" onClick={() => setShowFilters(false)}>
                  Apply Filters
                </Button>
              </div>
            </div>
        ...
        <div className="flex items-center justify-between mb-6">
          <p className="text-sm text-slate-500">
            Showing <span className="text-navy dark:text-white font-bold">{projects.length}</span> of <span className="text-navy dark:text-white font-bold">{totalCount}</span> projects
          </p>
        ...
        <div className="grid grid-cols-1 gap-6">
          {loading ? (
            Array(5).fill(0).map((_, i) => (
              <div key={i} className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-6 animate-pulse">
            ...
            projects.map((project) => (
              <ProjectCard key={project._id} project={project} onApply={() => handleApply(project._id)} />
            ))
          ) : (
            <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-3xl p-20 text-center">
              ...
              <h3 className="text-2xl font-bold text-navy dark:text-white mb-2">No projects found</h3>
              <p className="text-slate-500 max-w-md mx-auto">Try adjusting your filters or search query to find more results.</p>
              <Button 
                variant="outline" 
                className="mt-8"
                onClick={() => { setSearchQuery(""); setSelectedCategory("All Categories"); fetchProjects(); }}
              >
                Clear all filters
              </Button>
            </div>
        ...
        {!loading && totalCount > limit && (
          <div className="mt-12 flex items-center justify-center gap-2">
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: Math.min(5, Math.ceil(totalCount / limit)) }).map((_, i) => (
                <button
                  key={i}
...
const ProjectCard = ({ project, onApply }: { project: Project; onApply: () => void }) => {
  return (
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-y-3 gap-x-6 mb-5 text-[13px] sm:text-sm text-slate-600 dark:text-slate-400 font-bold">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="capitalize">{project.experienceLevel || 'Intermediate'}</span>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {project.requiredSkills.slice(0, 6).map((skill: string) => (
              <span key={skill} className="px-4 py-2 bg-slate-50 dark:bg-white/5 text-slate-600 dark:text-slate-200 rounded-xl text-[11px] font-bold border border-slate-100 dark:border-white/5 shadow-sm">
                {skill}
              </span>
            ))}
            {project.requiredSkills.length > 6 && (
              <span className="px-3 py-2 text-[11px] text-slate-400 font-black">+{project.requiredSkills.length - 6}</span>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-slate-100 dark:border-white/5">
           <div className="flex items-center gap-3">
             <div className="flex -space-x-3">
               {[1, 2, 3].map((i) => (
                 <div key={i} className="w-8 h-8 rounded-full border-2 border-white dark:border-[#050B15] bg-slate-100 dark:bg-white/10 flex items-center justify-center text-[10px] font-bold text-slate-400">
                   {String.fromCharCode(64 + i)}
                 </div>
               ))}
             </div>
             <p className="text-xs font-bold text-slate-500">
               <span className="text-navy dark:text-white">{project.applications || 0}</span> proposals
             </p>
           </div>
