...
const Pricing = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050B15] font-sans text-white">
      <PublicNavbar />
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10 text-center">
          <AnimatedSection>
            ...
            <h1 className="text-5xl md:text-7xl font-bold mb-8">
              Invest in Your <br />
              <span className="text-gradient">Professional Growth</span>
            </h1>
            ...
            <div className="flex items-center justify-center gap-6 mb-8">
              <span className={cn("text-sm font-bold transition-all", billingCycle === "monthly" ? "text-white" : "text-slate-500")}>Monthly</span>
              <button
                onClick={() => setBillingCycle(billingCycle === "monthly" ? "yearly" : "monthly")}
                className="w-16 h-8 bg-white/10 rounded-full p-1 relative transition-all border border-white/10"
              >
                <div className={cn(
              ...
              <div className="flex items-center gap-2">
                <span className={cn("text-sm font-bold transition-all", billingCycle === "yearly" ? "text-white" : "text-slate-500")}>Yearly</span>
                <span className="px-2 py-0.5 bg-teal/20 text-teal text-[10px] font-bold rounded-full border border-teal/30">Save 20%</span>
              </div>
            </div>
          </AnimatedSection>
        </div>
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto items-end">
            {plans.map((plan, idx) => (
              <AnimatedSection key={plan.name} delay={idx * 100}>
                <div className={cn(
                  "relative glass-card p-10 rounded-[2.5rem] transition-all duration-500 flex flex-col group",
                  plan.highlighted ? "border-teal/40 bg-white/10 py-14 shadow-teal/5" : "hover:bg-white/10 hover:border-white/20",
                )}>
                  {plan.badge && (
                    ...
                    {plan.features.map((feature, fIdx) => (
                      <li key={fIdx} className="flex items-center gap-3 text-sm">
                        {feature.included ? (
                          ...
                          <X size={16} className="text-white/10" />
                        )}
                        <span className={feature.included ? "text-slate-300" : "text-white/20"}>{feature.text}</span>
                      </li>
                    ))}
                  </ul>
      ...
      <section className="py-32 bg-white/5 border-y border-white/5">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="text-center mb-20">
            ...
            <p className="text-slate-500">Pick the perfect plan for your professional needs.</p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="p-8 text-sm font-bold text-slate-400 font-bold uppercase tracking-widest">Features</th>
              ...
              <tbody>
                {comparisonFeatures.map((f, i) => (
                  <tr key={i} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-colors">
                    <td className="p-8 text-slate-300 font-medium">{f.name}</td>
                    <td className="p-8 text-center">
                      {typeof f.free === 'boolean' ? (f.free ? <Check className="mx-auto text-teal" size={20} /> : <X className="mx-auto text-white/10" size={20} />) : <span className="text-sm text-slate-400">{f.free}</span>}
                    </td>
                    <td className="p-8 text-center">
                      {typeof f.pro === 'boolean' ? (f.pro ? <Check className="mx-auto text-teal" size={20} /> : <X className="mx-auto text-white/10" size={20} />) : <span className="text-sm font-bold text-teal">{f.pro}</span>}
                    </td>
                    <td className="p-8 text-center">
                      {typeof f.premium === 'boolean' ? (f.premium ? <Check className="mx-auto text-teal" size={20} /> : <X className="mx-auto text-white/10" size={20} />) : <span className="text-sm font-bold text-royal-blue">{f.premium}</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <section className="py-32">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-20">
              <span className="text-teal font-bold uppercase tracking-widest text-xs mb-4 block">Still Curious?</span>
              <h2 className="text-4xl font-bold">Frequently Asked Questions</h2>
            </div>
            <div className="space-y-4">
              {faqs.map((faq, idx) => (
                <div key={idx} className="glass-card rounded-2xl overflow-hidden border-white/5 transition-all">
                  <button
      ...
      <section className="py-40 relative">
        <div className="absolute inset-0 bg-gradient-to-t from-royal-blue/10 to-transparent pointer-events-none" />
        <div className="container mx-auto px-4 lg:px-8 relative z-10 text-center">
          <AnimatedSection>
            <h2 className="text-5xl md:text-7xl font-bold mb-10">Ready to <span className="text-gradient">Level Up?</span></h2>
            <div className="flex flex-col sm:flex-row gap-6 justify-center">
              <Link to="/register?role=freelancer">
                <Button size="lg" className="h-16 px-12 rounded-2xl bg-teal text-white font-bold text-lg hover:bg-[#128a7f] transition-all hover:scale-105 shadow-2xl shadow-teal/30">
                  Upgrade Today
                </Button>
              </Link>
              <Link to="/contact">
